...
## Formulating Language Generation as a Reinforcement Learning Problem

The key idea behind reinforcement learning is fundamentally different from supervised fine-tuning. Instead of training the model to imitate demonstrations, reinforcement learning allows the model to generate its own responses and improves the policy according to the quality of those responses. The learning signal is therefore no longer provided by the next token in a demonstration, but by a reward that evaluates the generated response.

To formulate language generation as a reinforcement learning problem, we first establish the correspondence between the concepts of language modeling and reinforcement learning. Consider an input prompt $$x$$ and let $$y=(y_1,y_2,\ldots,y_T)$$ denote the response generated by the language model. At each decoding step, the model observes the prompt together with all previously generated tokens before predicting the next token. From the perspective of reinforcement learning, the prompt together with the previously generated tokens constitutes the state

\begin{equation}
s_t=(x,y_{<t}),
\end{equation}

while the next generated token corresponds to the action

\begin{equation}
a_t=y_t.
\end{equation}

Beginning with the initial state $$s_0=(x,\emptyset)$$, the language model repeatedly samples the next token from the policy

\begin{equation}
a_t \sim \pi_\theta(a_t|s_t),
\end{equation}

until an end-of-sequence token is generated. The complete sequence of states and actions

\begin{equation}
\tau=(s_0,a_0,s_1,a_1,\ldots,s_T)
\end{equation}

is called a trajectory. In the context of language models, a trajectory simply represents the entire decoding process that produces a response.

Once the complete response has been generated, it is evaluated by a reward function that assigns a scalar reward $$R(\tau)$$ to the trajectory. Depending on the application, this reward may represent the correctness of the generated answer, adherence to a desired format, human preference, or the output of a learned reward model. Since the trajectory depends on the policy $$\pi_\theta$$, the reward is also a random variable. The objective of reinforcement learning is therefore to find the policy that maximizes the expected reward over all possible trajectories generated by the model.

In the next subsection, we derive the policy gradient objective used to optimize this expectation. This derivation forms the mathematical foundation of nearly every reinforcement learning algorithm discussed in the remainder of this article, including PPO, GRPO, and OPD.
